...
> **2026-08-18 evidence precedence note:** The body below preserves the earlier consolidated ROUND18EW-era reference for historical completeness. Newer supplied hardware evidence reaches **ROUND18FB**. For current claims, ROUND18FB is the latest architecture; ROUND18EO remains the peak verified clean 128-token E2E result (~3.411 tok/s). Recent ~3.5 GB/s values are logical graph traffic, not physical DRAM bandwidth. See `docs/18-latest-hardware-state.md`.
...
# 2026-08-18 Post-EW Evidence Addendum

The authoritative post-EW update is maintained in [`18-latest-hardware-state.md`](18-latest-hardware-state.md). It records ROUND18EX/EY/FA/FB, latest clean baseline results, guarded QKV promotion, pinned-mailbox Q6 qualification, measured boot time, GGTT static-zone observations, and corrected logical-throughput semantics.
